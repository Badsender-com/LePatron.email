...
| **Code mort**                       | `test-budget.service.js`, son test, `User.dailyTestInvocationCount` et `MaxDailyTestInvocations` supprimés — le test runner qui les consommait avait été retiré avant la PR. Le quota reste consommé par le playground : la PR 2/2 le remonte dans `ai-playground/` sous `MaxDailyPlaygroundRuns` (§4bis).                                                               |
