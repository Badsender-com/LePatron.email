...
describe('Stylesheet declaration processor', function () {
  it('should add "namespacing" to every rule', function () {
    ...
    expect(result).toEqual(
      "<!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a{ b: c }"
    );
    ...
    expect(result).toEqual(
      "<!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a, <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->b{ b: c } @media all { <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->c{ d: e } }"
    );
  ...
  it('should process simple -ko properties', function () {
    ...
    expect(result).toEqual(
      "<!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a{ b: c; b: <!-- ko text: $block.myc[c]() -->c<!-- /ko -->}"
    );
  ...
  it('should process [data-ko-block=] directive', function () {
    ...
    // function(style, rules, localWithBindingProvider, blockDefsUpdater, themeUpdater, basePath, rootModelName, templateName) {
    result = processStylesheetRules(
      'a[data-ko-block=foo] { b: c; -ko-b: @myc }',
      undefined,
      mockedWithBindingProvider,
      blockDefsUpdater,
      themeUpdater,
    ...
    expect(result).toEqual(
      "<!-- ko foreach: $root.findObjectsOfType($data, 'foo') --> <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a<!-- ko text: '#'+id() -->foo<!-- /ko -->{ b: c; b: <!-- ko text: $foo.myc[c]() -->c<!-- /ko --> } <!-- /ko -->"
    );

    expect(blockDefsUpdater).toHaveBeenCalledWith('foo', '', {
      contextName: 'block',
    });
    // blockDefsUpdater.calls.reset();

    result = processStylesheetRules(
      'a[data-ko-block=foo], [data-ko-block=foo] a { b: c; -ko-b: @myc }',
      undefined,
      mockedWithBindingProvider,
      blockDefsUpdater,
      themeUpdater,
    ...
    expect(result).toEqual(
      "<!-- ko foreach: $root.findObjectsOfType($data, 'foo') --> <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a<!-- ko text: '#'+id() -->foo<!-- /ko -->, <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko --><!-- ko text: '#'+id() -->foo<!-- /ko --> a{ b: c; b: <!-- ko text: $foo.myc[c]() -->c<!-- /ko --> } <!-- /ko -->"
    );
    ...
    result = processStylesheetRules(
      'a[data-ko-block=foo],\n [data-ko-block=foo] a { b: c; -ko-b: @myc }',
      undefined,
      mockedWithBindingProvider,
      blockDefsUpdater,
      themeUpdater,
    ...
    result = processStylesheetRules(
      'a[data-ko-block=foo]{}b[data-ko-block=foo]{}',
      undefined,
      mockedWithBindingProvider,
      blockDefsUpdater,
      themeUpdater,
    ...
    expect(result).toEqual(
      "<!-- ko foreach: $root.findObjectsOfType($data, 'foo') --> <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a<!-- ko text: '#'+id() -->foo<!-- /ko -->{} <!-- /ko --><!-- ko foreach: $root.findObjectsOfType($data, 'foo') --> <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->b<!-- ko text: '#'+id() -->foo<!-- /ko -->{} <!-- /ko -->"
    );
  ...
  it('should throw error on multiple different [data-ko-block=] directive', function () {
    ...
    try {
      result = processStylesheetRules(
        'a[data-ko-block=foo], b[data-ko-block=bar] { b: c; -ko-b: @myc }',
        undefined,
        mockedWithBindingProvider,
        blockDefsUpdater,
        themeUpdater,
  ...
  it('should process multiline [data-ko-block=] directive', function () {
    ...
    expect(result).toEqual(
      "    <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->f{ g: h; }\n    <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->i{j:k}\n    <!-- ko foreach: $root.findObjectsOfType($data, 'foo') -->\n    <!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a<!-- ko text: '#'+id() -->foo<!-- /ko -->{\n      b: c;\n      b: <!-- ko text: $foo.myc[c]() -->c<!-- /ko -->\n    }\n    <!-- /ko -->"
    );
  ...
  it('should ignore comments or unknown rules but keep them', function () {
    ...
    // function(style, rules, localWithBindingProvider, blockDefsUpdater, themeUpdater, basePath, rootModelName, templateName) {
    result = processStylesheetRules(
      '/* first */a { b: c; /* second */-ko-b: @myc }/* third */',
      undefined,
      mockedWithBindingProvider,
      undefined,
      undefined,
    ...
    expect(result).toEqual(
      "/* first */<!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a{ b: c; /* second */b: <!-- ko text: $block.myc[c]() -->c<!-- /ko -->}/* third */"
    );

    result = processStylesheetRules(
      '@keyframes{a{-ko-b:@c}}a {b:c}',
      undefined,
      mockedWithBindingProvider,
      undefined,
      undefined,
    ...
    expect(result).toEqual(
      "@keyframes{a{-ko-b:@c}}<!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a{b:c}"
    );
  });
  ...
  it('should parse unsupported supports rules', function () {
    var result;
    result = processStylesheetRules(
      '@supports cips {a{b:1;-ko-b:@myb}}a {b:c}',
      undefined,
      mockedWithBindingProvider,
      undefined,
      undefined,
    ...
    expect(result).toEqual(
      "@supports cips {<!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a{b:1;b: <!-- ko text: $block.myb[1]() -->1<!-- /ko -->}}<!-- ko text: templateMode =='wysiwyg' ? '#main-wysiwyg-area ' : '' --><!-- /ko -->a{b:c}"
    );
  });
  ...
  it('should parse -ko-blockdefs definitions', function () {
    ...
    var blockDefsUpdater = jasmine.createSpy('blockDefsUpdater');
    result = processStylesheetRules(
      '@supports -ko-blockdefs { color { widget: color } color:preview { -ko-color: @color } }',
      undefined,
      mockedWithBindingProvider,
      blockDefsUpdater,
      undefined,
    ...
    expect(blockDefsUpdater).toHaveBeenCalledWith('color', undefined, {
      previewBindings: [
        {
          type: 'property',
          name: '-ko-color',
  ...
  it('should parse -ko-blockdefs definitions, even quoted values', function () {
    ...
    var blockDefsUpdater = jasmine.createSpy('blockDefsUpdater');
    result = processStylesheetRules(
      '@supports -ko-blockdefs { color { label: "esc\'aped" } }',
      undefined,
      mockedWithBindingProvider,
      blockDefsUpdater,
      undefined,
  ...
  it('should raise an error when mixing preview selctor with other declarations', function () {
    ...
    try {
      result = processStylesheetRules(
        '@supports -ko-blockdefs { color, color:preview { } }',
        undefined,
        mockedWithBindingProvider,
        blockDefsUpdater,
        undefined,
