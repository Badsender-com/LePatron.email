...
if (!$.ui.version.match(/^1\.11\..*$/))
  throw (
    'Usupported jQuery UI version detected: ' +
    $.ui.version +
    ' (we only support 1.11.*)'
  );
  ...
  var newCallback = function (node) {
    try {
      ...
      if (typeof console.debug == 'function')
        console.debug('Caught unexpected dispose callback exception', e);
    }
  };
  ...
  return {
    viewModel: function (viewModel) {
      try {
        performanceAwareCaller(
          'applyBindings',
          ko.applyBindings.bind(undefined, viewModel)
        );
      } catch (err) {
    ...
    dispose: function () {
      try {
        performanceAwareCaller(
          'unapplyBindings',
          ko.cleanNode.bind(this, global.document.body)
        );
      } catch (err) {
        console.warn(err, err.stack);
        throw err;
...
var templateCreator = function (
  ...
  if (
    typeof optionalName != 'undefined' &&
    typeof templateMode != 'undefined'
  ) {
    if (
      typeof htmlOrElement != 'object' ||
      htmlOrElement.tagName.toLowerCase() != 'replacedhtml'
    )
      tmpName += '-' + templateMode;
  }

  while (
    typeof tmpName == 'undefined' ||
    tmpName === null ||
    templatePlugin.exists(tmpName)
  ) {
  ...
  return {
    ...
    init: function () {
      if (
        ...
        typeof instance.init !== 'undefined'
      )
        instance.init();
    },
    dispose: function () {
      if (
        typeof instance !== 'undefined' &&
        typeof instance.dispose !== 'undefined'
      )
        instance.dispose();
    },
  };
...
if (process.env.MOSAICO) {
  ...
  var templateLoader = function (
    ...
    var templateUrlConverter = _templateUrlConverter.bind(
      undefined,
      templatePath
    );
  ...
  var templateLoader = function (
    ...
    var templateUrlConverter =
      ko.bindingHandlers.wysiwygSrc.templateUrlConverter;
...
var templateCompiler = function (
  ...
  for (var ele in basicStructure)
    if (basicStructure.hasOwnProperty(ele))
      if (basicStructure[ele] != 1) {
        if (basicStructure[ele] === 0)
          throw 'ERROR: missing mandatory element ' + ele + '>';
        if (basicStructure[ele] > 1)
          throw (
            'ERROR: multiple element ' +
            ele +
            '> occourences are not supported (found ' +
            basicStructure[ele] +
            ' occourences)'
          );
      }
  ...
  var templateDef = performanceAwareCaller(
    'translateTemplate',
    templateConverter.translateTemplate.bind(
      undefined,
      templateName,
      html,
      templateUrlConverter,
  ...
  var content = performanceAwareCaller(
    'generateModel',
    templateConverter.wrappedResultModel.bind(undefined, templateDef)
  );
  ...
  blockDefs.push.apply(
    blockDefs,
    performanceAwareCaller(
      'generateEditors',
      templateConverter.generateEditors.bind(
        undefined,
        templateDef,
        widgets,
        templateUrlConverter,
  ...
  if (typeof jsorjson !== 'undefined' && jsorjson !== null) {
    ...
    var checkModelRes = performanceAwareCaller(
      'checkModel',
      templateConverter.checkModel.bind(
        undefined,
        content._unwrap(),
        blockDefs,
        unwrapped
    ...
    if (checkModelRes == 2) {
      console.error(
        'Trying to compile an incompatible template version!',
        content._unwrap(),
        blockDefs,
        unwrapped
      );
  ...
  var iframeTpl =
    prefix +
    templateSystem
      .getTemplateContent(templateName + '-iframe')
      .replace(
        /(<\/?)replaced(html|head|body)([^>]*>)/gi,
        function (match, p1, p2, p3) {
          return p1 + p2 + p3;
        }
      ) +
  ...
  // Palette contents. The synthetic blocks always go last, and each is hidden
  // altogether when its own template flag is off — they are independent, so a
  // client can have the builder without the raw HTML block. `blockDefs` itself
  // stays untouched: checkModel (above) must keep seeing every known block type,
  // or it would splice stored blocks out of the user's content.
  var paletteBlockDefs = syntheticBlockInjector.orderPaletteBlockDefs(
    blockDefs,
    metadata
  );
  ...
  if (
    typeof viewModel.metadata.editorversion !== 'undefined' &&
    viewModel.metadata.editorversion !== editver
  ) {
    console.log(
      'The model being loaded has been created with a different editor version',
      viewModel.metadata.editorversion,
      'runtime:',
      editver
    );
  }
  ...
  if (typeof templateDef.version !== 'undefined') {
    if (
      typeof viewModel.metadata.templateversion !== 'undefined' &&
      viewModel.metadata.templateversion !== templateDef.version
    ) {
      console.log(
        'The model being loaded has been created with a different template version',
        viewModel.metadata.templateversion,
        'runtime:',
        templateDef.version
      );
    }
...
var isCompatible = function () {
  try {
    ...
    checkFeature('ES5 strict', function () {
      return (function () {
        /* "use strict";*/ return typeof this == 'undefined';
      })();
    });
    ...
    checkFeature('CSS backgroundOrigin', function () {
      return (
        typeof global.document.body.style['backgroundOrigin'] != 'undefined'
      );
    });
...
var checkBadBrowserExtensions = function () {
  ...
  ko.bindingHandlers.bindIframe.tpl =
  ...
  if (expected !== content && expected2 !== content) {
    ...
    console.warn(
      'Detected incompatible/misbehaving browser, probably introduced by a bad browser extension.'
    );
