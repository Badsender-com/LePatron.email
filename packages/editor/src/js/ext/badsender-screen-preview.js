...
function screenPreview(viewModel) {
  ...
  viewModel.loadedTemplate.subscribe(initializeMediaQueries, viewModel, 'change')
  viewModel.previewMode.subscribe(getScreenVisibility, viewModel, 'change')
}

function setMediaQueries(mode, setSuffix = false) {
  cssMediaResources.forEach(({ stylesheet, cssMediaRuleIdx, baseMediaCondition }) => {
    const outputCondition = ['both', 'mobile'].includes(mode) ? desktopMediaQuery : baseMediaCondition
    let styles = ""
    ...
    stylesheet.insertRule(`@media ${outputCondition}{${styles}}`,cssMediaRuleIdx)
  });
}
