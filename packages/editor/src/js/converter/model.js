...
var badsenderHelpers = require('../badsender-helpers.js');
  ...
  if (dotPos == -1) {
    if (typeof model[prop] == 'undefined') {
      ...
      if (
        ...
        console.log('nullpropobjectvalue', prop, value);
      model[prop] = value;
    } else if (
      typeof model[prop] == 'object' &&
      typeof model[prop].push == 'function'
    ) {
      var values;
      if (typeof value === 'string') {
        ...
        if (valuesString !== null) {
          ...
          throw (
            'Unexpected default value for array property ' + prop + ': ' + value
          );
        }
      } else if (
        typeof value === 'object' &&
        ...
        throw (
          'Unexpected default value for array property ' +
          prop +
          ': ' +
          value +
          ' typeof ' +
      ...
      typeof model[prop] == 'object' &&
      model[prop] !== null &&
      typeof model[prop]._widget != 'undefined'
    ) {
      if (typeof value == 'object' && value !== null)
        console.log('objectvalue', prop, model[prop]._widget, value);
...
var _modelCreateOrUpdateBlockDef = function (
  defs,
  templateName,
  properties,
  namedProperties
  ...
  if (
    typeof defs[templateName] !== 'undefined' &&
    defs[templateName]._initialized &&
    !defs[templateName]._writeable
  ) {
    console.log(
      '_modelCreateOrUpdateBlockDef',
      defs,
      templateName,
      properties,
      namedProperties
  ...
  if (typeof defs[templateName] == 'undefined') {
    ...
    if (
      typeof namedProperties.category == 'undefined' &&
      typeof defs[templateName]._category == 'undefined'
    ) {
      if (
        templateName.match(/(^t|.T)heme$/) ||
        templateName.match(/(^s|.S)tyle$/) ||
        templateName.match(/(^c|.C)olor$/) ||
        templateName.match(/(^r|.R)adius$/)
      ) {
  ...
  if (typeof namedProperties !== 'undefined') {
    ...
    if (typeof namedProperties.name != 'undefined')
      defs[templateName]._name = namedProperties.name;
    ...
    if (typeof namedProperties.globalStyle != 'undefined') {
      ...
      var globalStyleSub = namedProperties.globalStyle.replace(
        /^(\$theme|_theme_)\./,
        ''
      );
      ...
      if (
        typeof defs[templateName]._themeOverride === 'undefined' ||
        !!defs[templateName]._themeOverride
      ) {
    ...
    if (typeof namedProperties.contextName !== 'undefined') {
      ...
      if (
        namedProperties.contextName == 'block' &&
        typeof defs[templateName]._globalStyle == 'undefined'
      ) {
    ...
    if (typeof namedProperties.extend != 'undefined')
      defs[templateName].type = namedProperties.extend;
  }

  for (var np in namedProperties)
    if (
      namedProperties.hasOwnProperty(np) &&
      typeof namedProperties[np] !== 'undefined' &&
      ['name', 'extend', 'contextName', 'globalStyle', 'themeOverride'].indexOf(
        np
      ) == -1
    ) {
  ...
  if (typeof properties != 'undefined' && properties.length > 0) {
    defs[templateName]._props =
      typeof defs[templateName]._props != 'undefined' &&
      defs[templateName]._props.length > 0
        ? defs[templateName]._props + ' ' + properties
        : properties;
  }
};
...
var _generateModelFromDef = function (modelDef, defs) {
  ...
  for (var prop in modelDef)
    if (!prop.match(/^_.*/) && modelDef.hasOwnProperty(prop)) {
      ...
      if (
        typeof value == 'object' &&
        value !== null &&
        typeof value._complex != 'undefined' &&
        value._complex
      ) {
  ...
  // when parsing integer are initialized to string
  // • this could generate bug when a template is making operation on this value
  //   like addition and so on…
  // • this is a quick for that
  return badsenderHelpers.ensureInteger(res);
};
...
var _getModelDef = function (defs, name, returnClone, readonly) {
  ...
  if (typeof defs[name] == 'undefined') {
    ...
    if (typeof defObj != 'object')
      throw (
        'Block definition must be an object: found ' + defObj + ' for ' + name
      );
    ...
    if (typeof defObj._initialized == 'undefined') {
      ...
      if (defObj.type != name && typeof defObj._widget == 'undefined') {
        ...
        typeof defObj._widget == 'undefined' &&
        typeof defObj._props == 'undefined' &&
        typeof defObj._complex == 'undefined'
      ) {
    ...
    if (returnClone) {
      ...
      if (
        typeof defObj._writeable == 'undefined' ||
        defObj._writeable === false
      )
        throw 'Retrieving non writeable object definition: ' + name;
      return defObj;
    }
  }
};
...
var _increaseUseCount = function (readonly, model) {
  if (!readonly) {
    ...
    console.error(
      'ERROR trying to bind an unused property while readonly',
      model
    );
...
var ensureGlobalStyle = function (
  defs,
  readonly,
  gsBindingProvider,
  modelName,
  ...
  var globalStyleBindingBindValue = gsBindingProvider(
    gsFullPath,
    defaultValue,
    overrideDefault
  );
  ...
  if (typeof defs[modelName]._globalStyles[path] == 'undefined') {
    if (readonly)
      throw 'Cannot find _globalStyle for ' + path + ' in ' + modelName + '!';
    if (
      path.indexOf('.') != -1 ||
      (typeof defs[modelName][path] == 'object' &&
        typeof defs[modelName][path]._widget !== 'undefined')
    ) {
      defs[modelName]._globalStyles[path] = globalStyleBindingBindValue;
    }
  } else if (defs[modelName]._globalStyles[path] != globalStyleBindingBindValue)
    throw (
      'Unexpected conflicting globalStyle [2] for ' +
      modelName +
      '/' +
      path +
      ': old=' +
      defs[modelName]._globalStyles[path] +
      ' new=' +
      globalStyleBindingBindValue
    );
};

// themeUpdater, defaultValue, overrideDefault, setcategory are only used in !readonly mode
var modelEnsurePathAndGetBindValue = function (
  readonly,
  defs,
  themeUpdater,
  rootModelName,
  ...
  if (typeof defs[modelName] === 'undefined')
    throw 'Cannot find model def for [' + modelName + ']';
  ...
  if (readonly) {
    if (
      typeof defs[modelName]._globalStyle !== 'undefined' &&
      typeof defs[modelName][propName] !== 'undefined' &&
      defs[modelName][propName]._category == 'style'
    ) {
  ...
  if (readonly) {
    if (typeof defaultValue !== 'undefined')
      throw 'Cannot use defaultValue in readonly mode!';
    if (overrideDefault)
      throw (
        'Cannot use overrideDefault in readonly mode for ' +
        modelName +
        '/' +
        path +
        '/' +
    ...
    if (defs[modelName]._writeable === false)
      console.log('TODO debug use cases for this condition', modelName, path);
  ...
  if (typeof model[propName] == 'undefined') {
    ...
    if (readonly)
      throw 'Cannot find path ' + propName + ' for ' + modelName + '!';
  ...
  try {
    ...
    if (propPos != -1) {
      ...
      if (
        typeof childModel[mypath] == 'undefined' ||
        childModel[mypath] === null
      ) {
        throw (
          'Found an unexpected path termination ' +
          mypath +
          ' for model ' +
          modelName +
          ' for ' +
    ...
    if (typeof childModel === 'undefined' || childModel === null)
      throw (
        'Unexpected null model for ' + modelName + '/' + within + '/' + fullPath
      );
    ...
    console.error(
      'TODO ERROR Property lookup exception',
      e,
      modelName,
      path,
      templateName,
  ...
  if (
    typeof defs[modelName]._globalStyle !== 'undefined' &&
    typeof defs[modelName][propName] == 'object' &&
    defs[modelName][propName] !== null &&
    typeof defs[modelName][propName]._category != 'undefined' &&
    defs[modelName][propName]._category == 'style'
  ) {
    // TODO can I restrict this code to !readonly mode?
    var gsBindingProvider = modelEnsurePathAndGetBindValue.bind(
      undefined,
      readonly,
      defs,
      themeUpdater,
    ...
    if (subPath.indexOf('.', 1) != -1)
      throw 'TODO unsupported object nesting! ' + path;

    var gsPath = defs[modelName]._globalStyle + '.' + propName;
    if (
      typeof defs[modelName][propName] == 'object' &&
      defs[modelName][propName] !== null &&
      typeof defs[modelName][propName]._globalStyle != 'undefined'
    ) {
    ...
    if (
      ...
      defaultValue = defs[modelName]._defaultValues[path];
    ...
    if (typeof defaultValue !== 'undefined') {
      if (readonly) {
        console.error(
          'Cannot set a new theme default value',
          gsFullPath.substr(7),
          defaultValue,
  ...
  if (typeof defaultValue != 'undefined') {
    if (
      typeof defs[modelName]._defaultValues[path] == 'undefined' ||
      (typeof overrideDefault != 'undefined' && overrideDefault)
    ) {
      if (readonly)
        throw (
          'Cannot set new _defaultValues [1] for ' +
          path +
          ' in ' +
          modelName +
          '!'
      ...
      if (defaultValue === null) {
        if (readonly && defs[modelName]._defaultValues[path] !== null) {
          throw (
            'Cannot set new _defaultValues [2] for ' +
            path +
            ' in ' +
            modelName +
            '!'
        ...
        console.error(
          'TODO error!!! Trying to set a new default value for ' +
            modelName +
            ' ' +
            path +
            ' while it already exists (current: ' +
            defs[modelName]._defaultValues[path] +
            ', new: ' +
            defaultValue +
            ')'
        );
        throw (
          'Trying to set a new default value for ' +
          modelName +
          ' ' +
          path +
          ' while it already exists (current: ' +
          defs[modelName].defaultValues[path] +
          ', new: ' +
          defaultValue +
          ')'
        );
...
module.exports = {
  ...
  ensurePathAndGetBindValue: modelEnsurePathAndGetBindValue.bind(
    undefined,
    false
  ),
