...
var checkModel = function (reference, blockDefs, model, origPrefix, reverse) {
  ...
  if (
    typeof blockDefs !== 'undefined' &&
    typeof blockDefs.splice == 'function'
  ) {
    blockDefsObj = {};
    for (i = 0; i < blockDefs.length; i++)
      blockDefsObj[blockDefs[i].type] = blockDefs[i];
  } else {
  ...
  for (var prop in reference)
    if (reference.hasOwnProperty(prop)) {
      prefix =
        typeof origPrefix !== 'undefined' ? origPrefix + '.' + prop : prop;
      if (!model.hasOwnProperty(prop)) {
        if (reverse) {
          console.warn(
            'WARN Property ',
            prefix,
            'found in model is not defined by template: removing it!'
          );
          ...
          console.log(
            'INFO Property ',
            prefix,
            'missing in model, cloning from reference!'
          );
        ...
        if (model[prop] !== null && reference[prop] !== null) {
          if (typeof model[prop] == 'string') {
            if (String(reference[prop]) != reference[prop]) {
              console.log(
                'TODO Different type 1 ',
                prefix,
                typeof model[prop],
                typeof reference[prop],
                model[prop],
                reference[prop]
              );
              valid = Math.max(valid, 2);
        ...
        if (reference[prop] !== null) {
          if (typeof reference[prop].splice !== 'undefined') {
              ...
              if (model[prop].length > 0) {
                ...
                for (i = 0; i < model[prop].length; i++) {
                  if (typeof model[prop][i].type == 'string') {
                    while (
                      j < reference[prop].length &&
                      reference[prop][j].type !== model[prop][i].type
                    ) {
                      console.log(
                        'ignoring ',
                        prefix,
                        reference[prop][j].type,
                        ' block type in reference not found in model'
                      );
                      j++;
                    }
                ...
                for (i = 0; i < reference[prop].length; i++) {
                  if (typeof reference[prop][i].type !== 'string') {
                    console.log(
                      'TODO found an object with no type',
                      prefix,
                      reference[prop][i]
                    );
                    valid = Math.max(valid, 2);
                  } else if (
                    !blockDefsObj.hasOwnProperty(reference[prop][i].type)
                  ) {
                    console.warn(
                      'TODO the model uses a block type not defined by the template. REMOVING IT!!',
                      prefix,
                      reference[prop][i]
                    );
                    ...
                    valid = Math.max(
                      valid,
                      checkModel(
                        blockDefsObj[reference[prop][i].type],
                        blockDefsObj,
                        reference[prop][i],
                        prefix + '[' + i + '.' + reference[prop][i].type + ']'
                      )
                    );
                  }
                }
            ...
            if (model[prop] === null) {
              if (reverse) {
                ...
                console.log(
                  'INFO Null object in model ',
                  prefix,
                  'instead of',
                  reference[prop],
                  'cloning it from the reference'
                );
                valid = Math.max(valid, 1);
                model[prop] = reference[prop];
              }
          ...
          console.log(
            'TODO Null in reference but not null in model',
            prefix,
            model[prop]
          );
          valid = Math.max(valid, 2);
        ...
        typeof reference[prop] !== 'boolean' &&
        typeof reference[prop] !== 'number'
      ) {
  ...
  if (!reverse)
    valid = Math.max(
      ...
      checkModel(
        ...
        blockDefs,
        reference,
        typeof origPrefix !== 'undefined' ? origPrefix + '!R' : '!R',
        true
      )
    );
  return valid;
