...
var _declarationValueLookup = function (
  declarations,
  propertyname,
  ...
  for (var i = declarations.length - 1; i >= 0; i--) {
    if (
      declarations[i].type == 'property' &&
      declarations[i].name == propertyname
    ) {
      return _declarationValueUrlPrefixer(
...
var elaborateDeclarations = function (
  ...
  var newBindings =
    typeof basicBindings == 'object' && basicBindings !== null
      ? basicBindings
      : {};
  var newStyle = null;
  var skipLines = 0;
    ...
    if (declarations[i].type == 'property') {
      if (
        ...
        if (decl !== null) {
          ...
          if (newStyle === null && typeof style != 'undefined')
            newStyle = style;
          ...
          if (isIf) {
            condDecl = declarations[i].name.substr(
              0,
              declarations[i].name.length - decl[3].length
            );
            var conditionedDeclaration = _declarationValueLookup(
              declarations,
              condDecl,
              templateUrlConverter
            );
            if (conditionedDeclaration === null)
              throw (
            ...
            if (
              ...
              throw 'Attributes and bind declarations are only allowed in inline styles!';
            ...
            if (isAttr) {
              ...
              if (needDefaultValue)
                propDefaultValue = _declarationValueLookup(
                  declarations,
                  propName,
                  templateUrlConverter
                );
            ...
            if (
              bindType !== null &&
              typeof newBindings[bindType] == 'undefined'
            )
              newBindings[bindType] = {};
            ...
            var declarationCondition = _declarationValueLookup(
              declarations,
              declarations[i].name + '-if',
              templateUrlConverter
            );
            var not = false;
            if (declarationCondition === null) {
              declarationCondition = _declarationValueLookup(
                declarations,
                declarations[i].name + '-ifnot',
                templateUrlConverter
              );
              not = true;
            } else {
              if (
                _declarationValueLookup(
                  declarations,
                  declarations[i].name + '-ifnot',
                  templateUrlConverter
                ) !== null
            ...
            if (declarationCondition !== null) {
              try {
                var bindingCond = converterUtils.conditionBinding(
                ...
                console.error(
                  'Unable to deal with -ko style binding condition',
                  declarationCondition,
                  declarations[i].name
                );
                throw e;
          ...
          // prefixing urls
          var replacedValue = _declarationValueUrlPrefixer(
            declarations[i].value,
            templateUrlConverter
          );
            ...
            if (newStyle !== null) {
              try {
                newStyle = converterUtils.removeStyle(
                  ...
                  0,
                  0,
                  declarations[i].name + ': ' + replacedValue
                );
              } catch (e) {
                console.log(
                  'Remove style failed replacing url',
                  e,
                  'name',
                  declarations[i]
                );
          ...
          var bindVal2 =
            typeof newBindings['virtualStyle'] !== 'undefined'
              ? newBindings['virtualStyle'][bindName2]
              : undefined;
          ...
          if (typeof bindVal2 !== 'undefined') {
            ...
            newBindings[bind] =
              ...
              declarations[i].name +
              ': ' +
              converterUtils.addSlashes(replacedValue) +
              ';' +
              dist +
              "'+" +
              newBindings[bind];
  ...
  if (typeof element != 'undefined' && element !== null) {
    for (var prop in newBindings['virtualStyle'])
      if (newBindings['virtualStyle'].hasOwnProperty(prop)) {
        console.log(
          'Unexpected virtualStyle binding after conversion to virtualAttr.style',
          prop,
          newBindings['virtualStyle'][prop],
          style
        );
        throw (
          'Unexpected virtualStyle binding after conversion to virtualAttr.style for ' +
          prop
        );
      }
    ...
    var dataBind =
      ...
      _bindingSerializer(newBindings);
  ...
  for (var prop in val)
    if (val.hasOwnProperty(prop)) {
      if (typeof val[prop] == 'object')
        res.push(prop + ': ' + '{ ' + _bindingSerializer(val[prop]) + ' }');
