...
var wrapElementWithCondition = function (attribute, element, bindingProvider) {
  ...
  try {
    ...
    console.warn(
      'Model ensure path failed in if/variant',
      element,
      cond,
      attribute
    );
...
var replacedAttributes = function (element, attributeName) {
  domutils.setAttribute(
    element,
    attributeName,
    domutils.getAttribute(element, 'replaced' + attributeName)
  );
};
...
var processStyle = function (
  ...
  var removeDisplayNone =
    domutils.getAttribute(element, 'data-ko-display') !== null;

  newStyle = elaborateDeclarations(
    style,
    undefined,
    templateUrlConverter,
    bindingProvider,
...
var processBlock = function (
  ...
  try {
    ...
    var fixedBlocks = $('[data-ko-block]', element).replaceWith(
      '<replacedblock>'
    );
    ...
    for (var i = 0; i < urlattrs.length; i++) {
      ...
      var func = _fixRelativePath.bind(
        undefined,
        urlattrs[i],
        templateUrlConverter
      );
    ...
    $('[data-ko-properties]', element).each(function (index, element) {
      ...
      dataDefs =
        dataDefs + domutils.getAttribute(element, 'data-ko-properties');
      domutils.removeAttribute(element, 'data-ko-properties');
    });

    modelDef.createOrUpdateBlockDef(defs, templateName, dataDefs, {
      contextName: contextName,
    });

    var bindingProvider = modelDef.ensurePathAndGetBindValue.bind(
      undefined,
      defs,
      themeUpdater,
      rootModelName,
    ...
    $('style', element).each(function (index, element) {
      ...
      var blockDefsUpdater = modelDef.createOrUpdateBlockDef.bind(
        undefined,
        defs
      );
      var localWithBindingProvider = modelDef.ensurePathAndGetBindValue.bind(
        undefined,
        defs,
        themeUpdater,
        rootModelName
      );
      var newStyle = processStylesheetRules(
        style,
        undefined,
      ...
      if (newStyle != style) {
        if (newStyle.trim() !== '') {
          ...
          domutils.setAttribute(element, 'title', `template-stylesheet`);
    ...
    for (var j = 0; j < notsupported.length; j++) {
      ...
      if (attr) {
        console.warn(
          'ERROR: Unsupported ' +
            notsupported[j] +
            ' used together with data-ko-block',
          element
        );
        throw (
          'ERROR: Unsupported ' +
    ...
    $('[data-ko-link]', element).each(function (index, element) {
      ...
      if (repStyle !== '')
        repStyle = '-ko-attr-href: @' + urlVar + '; ' + repStyle;
    ...
    $('[data-ko-editable]', element).each(function (index, element) {
      ...
      selectBinding =
        'wysiwygClick: function(obj, evt) { $root.selectItem(' +
        itemBindValue +
        ', $data); return false }, clickBubble: false, wysiwygCss: { selecteditem: $root.isSelectedItem(' +
        itemBindValue +
      ...
      if (domutils.getLowerTagName(element) != 'img') {
        ...
        if (!domutils.getAttribute(element, 'id')) {
          newBinding +=
            "wysiwygId: id()+'_" + dataEditable.replace('.', '_') + "', ";
        }
        ...
        if (domutils.getLowerTagName(element) == 'td') {
          var wrappingDiv = $(
            '<div data-ko-wrap="false" style="width: 100%; height: 100%"></div>'
          )[0];
          domutils.setAttribute(wrappingDiv, 'data-bind', newBinding);
          var newContent = domutils.getInnerHtml(
            $('<div></div>').append(wrappingDiv)
          );
          domutils.setContent(element, newContent);
        } else {
          currentBindings = domutils.getAttribute(element, 'data-bind');
          dataBind =
            (currentBindings !== null ? currentBindings + ', ' : '') +
            newBinding;
        ...
        if (width === null) {
          console.error(
            'ERROR: data-ko-editable images must declare a WIDTH attribute!',
            element
          );
        ...
        var dynHeight =
          currentBindings &&
          currentBindings.match(/virtualAttr: {[^}]* height: ([^,}]*)[,}]/);
        if (dynHeight) height = dynHeight[1];
        var dynWidth =
          currentBindings &&
          currentBindings.match(/virtualAttr: {[^}]* width: ([^,}]*)[,}]/);
        if (dynWidth) width = dynWidth[1];

        var method;

        defaultValue = domutils.getAttribute(
          element,
          'data-ko-placeholder-src'
        );
        ...
        var plheight =
          height ||
          domutils.getAttribute(element, 'data-ko-placeholder-height');
        var plwidth =
          width || domutils.getAttribute(element, 'data-ko-placeholder-width');
        ...
        if (!plwidth || !plheight) {
          ...
          console.error(
            'IMG data-ko-editable must declare width and height attributes, or their placeholder counterparts data-ko-placeholder-width/data-ko-placeholder-height',
            element
          );
        ...
        var bindingValue = bindingProvider(
          dataEditable,
          value,
          false,
          'wysiwyg'
        ...
        newBinding =
          ...
          ' }';
        dataBind =
          (currentBindings !== null ? currentBindings + ', ' : '') + newBinding;
        domutils.setAttribute(element, 'data-bind', dataBind);
        ...
        $(element).before(
          '<!-- ko wysiwygImg: { _data: $data, _item: ' +
            ...
            ", _template: '" +
            tmplName +
            "', _editTemplate: 'img-wysiwyg', _src: " +
            bindingValue +
            ', _width: ' +
            width +
            ', _height: ' +
    ...
    $('[href]', element).each(function (index, element) {
      ...
      var newBinding =
        "wysiwygHref: '" + converterUtils.addSlashes(attrValue) + "'";
    ...
    $('replacedblock', element).each(function (index, element) {
      ...
      var modelBindValue = modelDef.ensurePathAndGetBindValue(
        defs,
        themeUpdater,
        rootModelName,
        templateName,
      ...
      $(element).before(
        '<!-- ko block: { data: ' +
          converterUtils.addSlashes(modelBindValue) +
          ", template: 'block' } -->"
      );
      $(element).after('<!-- /ko -->');
      $(element).remove();
    ...
    $($('[data-ko-wrap]', element).get().reverse(), element).each(function (
      index,
      element
    ) {
    ...
    */
      ...
      if (
        ...
        domutils.replaceHtml(
          element,
          "<!-- ko template: /* special */ (typeof templateMode != 'undefined' && templateMode == 'wysiwyg') || " +
            condBinding +
            " ? '" +
            outerTmplName +
            "' : '" +
        ...
        domutils.setContent(
          element,
          "<!-- ko template: '" + innerTmplName + "' --><!-- /ko -->"
        );
        outerTmplName = templateCreator(element);
        domutils.replaceHtml(
          element,
          "<!-- ko template: (typeof templateMode != 'undefined' && templateMode == 'wysiwyg') || " +
            condBinding +
            " ? '" +
            outerTmplName +
            "' : '" +
...
function conditional_replace(html) {
  return html.replace(
    ...
    function (match, condition, body) {
      var dd = '<!-- cc:start -->';
      dd += body
        .replace(/<([A-Za-z:]+)/g, '<!-- cc:bo:$1 --><cc') // before open tag
        .replace(/<\/([A-Za-z:]+)>/g, '<!-- cc:bc:$1 --></cc><!-- cc:ac:$1 -->') // before/after close tag
        .replace(/\/>/g, '/><!-- cc:sc -->'); // self-close tag
      dd += '<!-- cc:end -->';
      var output =
        '<replacedcc condition="' + condition + '" style="display: none">';
      output += $('<div>')
        .append($(dd))
        .html()
        .replace(/^<!-- cc:start -->/, '')
...
var translateTemplate = function (
  ...
  var defs = {};
  var replacedHtml = conditional_replace(
    html.replace(
      /(<[^>]+\s)(style|http-equiv)(="[^"]*"[^>]*>)/gi,
  ...
  var content =
    typeof $.parseHTML == 'function'
      ? $($.parseHTML(replacedHtml, false))
      : $(replacedHtml);
  var element = content[0];

  var blocks = []; // {rootName, blockName, containerName}
  var _blockPusher = function (
    rootName,
    blockName,
    contextName,
    containerName
  ...
  var themeUpdater = function (name, key, val) {
    ...
    if (
      typeof defs['themes'][name][key] === 'undefined' ||
      defs['themes'][name][key] === null
    )
      ...
      if (precVal != val)
        console.log(
          'Error setting a new default for property ' +
            key +
            ' in theme ' +
            name +
            '. old:' +
  ...
  containers.each(function (index, element) {
    var containerName =
      domutils.getAttribute(element, 'data-ko-container') + 'Blocks';
  ...
  for (var prop in containersDom)
    if (containersDom.hasOwnProperty(prop)) {
      ...
      modelDef.ensurePathAndGetBindValue(
        defs,
        themeUpdater,
        templateName,
        templateName,
