...
ColorPicker.prototype = kojqui.utils.createObject(
  kojqui.BindingHandler.prototype
);
...
ColorPicker.prototype.init = function (element, valueAccessor, allBindings) {
  ...
  ko.computed({
    read: function () {
      ...
      for (var prop in va)
        if (prop !== 'color' && va.hasOwnProperty(prop))
          opt[prop] = ko.utils.unwrapObservable(va[prop]);
