...
ko.bindingHandlers.wysiwygOrHtml = {
  init: function (
    ...
    if (isNotWysiwygMode) return ko.bindingHandlers['virtualHtml'].init();
    else
      return ko.bindingHandlers.wysiwyg.init(
        element,
        valueAccessor,
        allBindingsAccessor,
        viewModel,
  ...
  update: function (
    ...
    var isNotWysiwygMode =
      ...
      bindingContext.templateMode != 'wysiwyg';
    if (isNotWysiwygMode)
      return ko.bindingHandlers['virtualHtml'].update(
        element,
        valueAccessor,
        allBindingsAccessor,
        viewModel,
    ...
    if (element.nodeType !== 8) {
      ...
      if (isNotWysiwygMode) {
        if (
          ...
          element.removeAttribute('href');
        else element.setAttribute('href', attrValue.toString());
      }
    }
  },
};
...
ko.bindingHandlers.wysiwygImg = {
  makeTemplateValueAccessor: function (valueAccessor, bindingContext) {
    return function () {
      ...
      return {
        name: isWysiwygMode
          ? unwrappedValue['_editTemplate']
          : unwrappedValue['_template'],
  ...
  init: function (
    ...
    return ko.bindingHandlers['template']['init'](
      element,
      ko.bindingHandlers['wysiwygImg'].makeTemplateValueAccessor(
        valueAccessor,
        bindingContext
      )
    );
...
var _catchingFire = function (event, args) {
  try {
    ...
    console.warn(
      'Cought tinymce exception while firing editor event',
      event,
      e
    );
  }
...
ko.bindingHandlers.wysiwyg = {
  ...
  fullOptions: {
    toolbar1:
      'bold italic forecolor backcolor hr styleselect removeformat | link unlink | pastetext code',
  ...
  init: function (
    ...
    var doDebug =
      ko.bindingHandlers.wysiwyg.debug && typeof console.debug == 'function';
    ...
    if (!ko.isObservable(value))
      throw 'Wysiwyg binding called with non observable';
    if (element.nodeType === 8)
      throw (
        'Wysiwyg binding called on virtual node, ignoring....' +
        element.innerHTML
      );
      ...
      init_instance_callback: function (editor) {
        ...
        if (ko.bindingHandlers.wysiwyg.initializingClass) {
          element.classList.remove(
            ko.bindingHandlers.wysiwyg.initializingClass
          );
        }
        ...
        editor.on('change redo undo', function () {
          if (!isSubscriberChange) {
            try {
              ...
              value(
                editor.getContent(ko.bindingHandlers.wysiwyg.getContentOptions)
              );
            } catch (e) {
              console.warn(
                'Unexpected error setting content value for',
                selectorId,
                e
              );
            } finally {
      ...
      function () {
        ...
        if (!isEditorChange) {
          try {
            ...
            console.warn(
              'Exception setting content to editable element',
              typeof thisEditor,
              e
            );
          }
