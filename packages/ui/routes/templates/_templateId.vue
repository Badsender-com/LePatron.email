  ...
  methods: {
    ...
    async deleteTemplate() {
      ...
      try {
        ...
        this.showSnackbar({ text: this.$t('snackbars.deleted'), color: `success` })
    ...
    async deleteImages() {
      ...
      try {
        ...
        this.showSnackbar({ text: this.$t('template.imagesRemoved'), color: `success` })
        this.template = template
      } catch (error) {
        this.showSnackbar({ text: this.$t('global.errors.errorOccured'), color: `error` })
        console.log(error)
...
<template>
  <bs-layout-left-menu>
    ...
    <v-row>
      <bs-template-edit-form v-model="template" :disabled="loading" @submit="updateTemplate" />
      <v-col cols="12" v-if="template.hasMarkup">
        <bs-template-html-preview :markup="template.markup" :template-id="template.id" />
      </v-col>
