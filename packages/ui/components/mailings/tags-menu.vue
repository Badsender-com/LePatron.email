...
export default {
  ...
  computed: {
    ...
    tagNameErrors() {
      ...
      !this.$v.newTagName.required && errors.push(this.$t('global.errors.nameRequired'))
      return errors
    },
    mailingsTags() {
...
<template>
  ...
  <v-menu v-model="vShowTagMenu" :close-on-content-click="false" :nudge-width="300">
    ...
    <v-card>
      <v-list>
        <v-list-item>
          <v-list-item-content>
            <v-list-item-title class="title">{{ $t(`tags.list`) }}</v-list-item-title>
          </v-list-item-content>
          <v-list-item-action>
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn color="primary" text @click="openNewTagDialog">{{ $t(`tags.new`) }}</v-btn>
        <v-btn color="primary" @click="updateMailingsTags">{{ $t(`global.apply`)}}</v-btn>
      </v-card-actions>
    </v-card>
    <v-dialog v-model="newTagDialog" width="500">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn text @click="closeNewTagDialog">{{ $t(`global.cancel`) }}</v-btn>
          <v-btn color="primary" @click="onCreateNewTag">{{ $t(`global.create`) }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-menu>
