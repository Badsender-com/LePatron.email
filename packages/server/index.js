...
//----- SESSION & I18N
  ...
  app.get(
      ...
      asyncHandler(passport.authenticate('saml', {failureRedirect: '/', failureFlash: true})),
      asyncHandler((err, user, info) => {
        console.log({err})
        if (err) {
          return res.redirect('/')
        }
...
// Mosaico's editor route
  ...
  app.use(function apiErrorHandler(err, req, res, next) {
    logger.error('Unexpected Error thrown in the application');
    logger.error(err)
    // delete err.config;
